...
function template_main()
{
	...
	<form action="', $scripturl, '?action=search2" method="post" accept-charset="', $context['character_set'], '" name="searchform" id="searchform">
		<h3 class="header_name">', $txt['set_parameters'], '</h3>';

	if (!empty($context['search_errors']))
		echo '
		<p id="search_error" class="information red">', implode('<br />', $context['search_errors']['messages']), '</p>';
...
function template_results()
{
	...
	<div id="search_results">
		<h3 class="header_name">', $txt['search_adjust_query'], '</h3>
		<div class="roundframe">';

		// Did they make any typos or mistakes, perhaps?
		if (isset($context['did_you_mean']))
			echo '
			<p class="teal">', $txt['search_did_you_mean'], ' <a href="', $scripturl, '?action=search2;params=', $context['did_you_mean_params'], '">', $context['did_you_mean'], '</a>.</p>';
	...
	echo '
		...
		while ($topic = $context['get_topics']())
		{
			...
			{
				echo '
		<ul class="reset a_topics_single a_result' , $options['display_quick_mod'] != 1 ? ' boxes' : '' , '">
			<li class="count">', $message['counter'], '</li>
			<li class="subject">
				', $topic['board']['link'], ' / <a href="', $scripturl, '?topic=', $topic['id'], '.msg', $message['id'], '#msg', $message['id'], '">', $message['subject_highlighted'], '</a>
				',$txt['by'],' <strong>', $message['member']['link'], '</strong> ',$txt['on'],' <em>', $message['time'], '</em>
			</li>';
			...
			<li class="moderation">';
				...
				<input type="checkbox" name="topics[]" value="', $topic['id'], '" class="input_check" />';
					else
					{
						if ($topic['quick_mod']['remove'])
							echo '
				<a href="', $scripturl, '?action=quickmod;actions[', $topic['id'], ']=remove;', $context['session_var'], '=', $context['session_id'], '" onclick="return confirm(\'', $txt['quickmod_confirm'], '\');"><span class="icon-trash" title="', $txt['remove_topic'], '"></span></a>';
						if ($topic['quick_mod']['lock'])
							echo '
				<a href="', $scripturl, '?action=quickmod;actions[', $topic['id'], ']=lock;', $context['session_var'], '=', $context['session_id'], '" onclick="return confirm(\'', $txt['quickmod_confirm'], '\');"><span class="icon-lock" title="', $txt['set_lock'], '"></span></a>';
						if ($topic['quick_mod']['sticky'])
							echo '
				<a href="', $scripturl, '?action=quickmod;actions[', $topic['id'], ']=sticky;', $context['session_var'], '=', $context['session_id'], '" onclick="return confirm(\'', $txt['quickmod_confirm'], '\');"><span class="icon-pin-outline" title="', $txt['set_sticky'], '"></span></a>';
						if ($topic['quick_mod']['move'])
							echo '
				<a href="', $scripturl, '?action=movetopic;topic=', $topic['id'], '.0"><span class="icon-forward-outline" title="', $txt['move_topic'], '"></span></a>';
					}
					echo '
			</li>';
				}
				...
				if ($message['body_highlighted'] != '')
					echo '
			<li class="body">', $message['body_highlighted'], '</li>';
				else
					echo '
			<li class="body">&nbsp;</li>';
	...
	{
		echo '
		<h3 class="header_name">', $txt['mlist_search_results'],':&nbsp;',$context['search_params']['search'],'</h3>
		<div class="pagesection">
			<div class="pagelinks">', $context['page_index'], '</div>
		</div>';
		...
		while ($topic = $context['get_topics']())
		{
			...
			{
				echo '
		<ul class="reset a_topics_single a_result mess">
			<li class="count">', $message['counter'], '</li>
			<li class="subject">
			...
			<li class="moderation">
				...
				if ($topic['can_reply'])
					echo '
					<li class="button_submit buts"><a href="', $scripturl . '?action=post;topic=' . $topic['id'] . '.' . $message['start'], '">', $txt['reply'], '</a></li>';
				if ($topic['can_quote'])
					echo '
					<li class="button_submit buts"><a href="', $scripturl . '?action=post;topic=' . $topic['id'] . '.' . $message['start'] . ';quote=' . $message['id'] . '">', $txt['quote'], '</a></li>';
				if ($topic['can_mark_notify'])
					echo '
					<li class="button_submit buts"><a href="', $scripturl . '?action=notify;topic=' . $topic['id'] . '.' . $message['start'], '">', $txt['notify'], '</a></li>';
